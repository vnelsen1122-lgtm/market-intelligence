...
function marketBattlecardFrame(market: string) {
  const normalized = market.toLowerCase();
  if (market === "All markets") return { angle: "Lead with the buyer's operating model, required workflows and proof standards before making a broad platform comparison.", priorities: ["Incident management", "Audits and inspections", "Training", "Analytics and reporting"] };
  if (/construction|contractor|infrastructure/.test(normalized)) return { angle: "Center the comparison on frontline adoption, mobile field execution, subcontractor coordination and fast incident-to-corrective-action workflows.", priorities: ["Incident management", "Mobile inspections", "Training", "Contractor management"] };
  if (/manufactur|chemical|food|metal/.test(normalized)) return { angle: "Center the comparison on plant-level workflow depth, repeatability across sites, chemical visibility and operational reporting.", priorities: ["Incident management", "Chemical and SDS", "Audits and inspections", "Analytics"] };
  if (/oil|gas|energy|utilit|mining|renewable/.test(normalized)) return { angle: "Center the comparison on high-hazard work, permit and control workflows, contractor exposure and multi-site regulatory complexity.", priorities: ["Operational risk", "Contractor management", "Environmental compliance", "Incident management"] };
  if (/waste|water|environment/.test(normalized)) return { angle: "Center the comparison on environmental obligations, field operations, permit evidence and recurring inspection workflows.", priorities: ["Environmental compliance", "Inspections", "Incident management", "Analytics"] };
  if (/health/.test(normalized)) return { angle: "Center the comparison on workforce scale, training consistency, incident response and simple adoption across distributed facilities.", priorities: ["Training", "Incident management", "Audits", "Analytics"] };
  return { angle: `Frame the comparison around ${market} operating conditions, buyer roles and the workflows that require the strongest proof.`, priorities: ["Incident management", "Audits and inspections", "Training", "Reporting"] };
}
...
export function CompetitorWorkspace({ selectedCompetitorId, onSelectCompetitor }: { selectedCompetitorId: string; onSelectCompetitor: (id: string) => void }) {
  ...
  const [battlecardMarket, setBattlecardMarket] = useState("All markets");
  ...
  const battlecardMarkets = unique(["All markets", ...(intelligence?.industries ?? selectedCompetitor.marketRelevance)]);
  const battlecardFrame = marketBattlecardFrame(battlecardMarket);
  const frictionSearchUrl = `https://www.google.com/search?q=${encodeURIComponent(`site:reddit.com OR site:g2.com ${selectedCompetitor.name} software reviews implementation pricing problems`)}`;
  ...
  const resetView = () => {
    ...
    setBattlecardMarket("All markets");
  ...
  const exportBattlecard = () => {
    ...
    const products = (intelligence?.productDepth ?? []).map((item) => `- ${item.family} — ${item.depth}: ${item.assessment}`).join("\n") || selectedCompetitor.modules.map((item) => `- ${item}`).join("\n");
    const customers = (intelligence?.customerProof ?? []).map((item) => `- ${item.customer} (${item.industry}): ${item.outcome}\n  - Source: ${item.sourceUrl}`).join("\n") || "- No named customer proof is mapped for this export.";
    download(`${selectedCompetitor.id}-${battlecardMarket.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-battle-card.md`, `# ${selectedCompetitor.name} — ${battlecardMarket} Battle Card\n\n**Exported:** ${new Date().toISOString()}\n**Research status:** ${intelligence?.researchStatus ?? "Source map only"}\n**Target market:** ${battlecardMarket}\n**Confidential layer:** Excluded from this public-safe export\n\n## Executive framing\n${battlecardFrame.angle}\n\n### Priority workflows for this market\n${battlecardFrame.priorities.map((item) => `- ${item}`).join("\n")}\n\n## Market posture\n- Motion: ${posture.motion}\n- Tier: ${posture.tier}\n- Strongest context: ${posture.strength}\n\n## Why they win\n${why}\n\n## Pressure points to validate\n${pressure}\n\n## How Novara can compete\n${novaraBaseline.strengths.map((item) => `- Lead with evidence of ${item.toLowerCase()} when it is relevant to ${battlecardMarket}.`).join("\n")}\n\nThese are Novara company statements and discovery frames, not approved comparative claims.\n\n## Product and workflow evidence\n${products}\n\n## Customer evidence\n${customers}\n\n## Questions to test\n${questions}\n\n## Sources\n${sources}\n`);
  };
  ...
  return <section className="ci-suite">
    ...
    {activeTab === "Overview" && <div className="ci-command-grid">
      <article className="panel ci-decision-room ci-win-command">
        <header><div><span className="section-label">Competitive decision</span><h2>How to position against {selectedCompetitor.name}</h2></div><mark>{intelligence ? `Reviewed ${intelligence.sources[0]?.observedAt ?? selectedCompetitor.retrieved}` : "Research required"}</mark></header>
        <div className="ci-decision-columns">
          <section className="tone-purple"><span className="ci-section-icon"><Radar size={14} /></span><b>Why they win</b>{(intelligence?.whyTheyWin ?? [{ claim: selectedCompetitor.statedPositioning, basis: "Official company positioning; deeper corroboration required.", sourceUrl: selectedCompetitor.officialUrl }]).map((item) => <details key={item.claim}><summary>{item.claim}</summary><p>{item.basis}</p><a href={item.sourceUrl} target="_blank" rel="noreferrer">View evidence <ArrowUpRight size={11} /></a></details>)}</section>
          <section className="tone-red"><span className="ci-section-icon"><AlertTriangle size={14} /></span><b>Friction signals</b>{(intelligence?.pressurePoints ?? [{ signal: "Pricing, implementation, module depth and customer outcomes have not been researched.", boundary: "Unknown, not a verified weakness." }]).map((item) => <details key={item.signal}><summary>{item.signal}</summary><p>{item.boundary}</p></details>)}<a className="ci-rerun-search" href={frictionSearchUrl} target="_blank" rel="noreferrer"><RefreshCw size={11} /> Rerun friction search</a></section>
          <section className="tone-green"><span className="ci-section-icon"><ShieldCheck size={14} /></span><b>How Novara wins</b><div className="ci-win-list">{novaraBaseline.strengths.slice(0, 4).map((item) => <span key={item}><ArrowRight size={11} />Lead with evidence of {item.toLowerCase()} when it matches the buyer’s requirements.</span>)}</div><button className="ci-open-brief" onClick={() => setActiveTab("Sales brief")}>Open market-ready sales brief <ArrowRight size={11} /></button></section>
          <section className="tone-blue"><span className="ci-section-icon"><Bot size={14} /></span><b>AI posture</b><div className="ci-ai-card"><strong>{intelligence?.ai.label ?? (selectedDomains.includes("AI") ? "AI referenced" : "No researched AI position")}</strong><p>{intelligence?.ai.summary ?? "A source-specific AI research pass is required before comparison."}</p>{intelligence?.ai.capabilities.map((capability) => <mark key={capability}>{capability}</mark>)}{intelligence?.ai.sourceUrl && <a href={intelligence.ai.sourceUrl} target="_blank" rel="noreferrer">Open AI source <ArrowUpRight size={11} /></a>}</div></section>
        </div>
      </article>

      <article className="panel ci-company-market-card"><header><div><span className="section-label">Company profile</span><h2>Position, footprint and market evidence</h2></div>{industryFocus !== "All industries" && <button className="ci-industry-reset" onClick={() => setIndustryFocus("All industries")}><ArrowRight size={12} /> Back to all markets</button>}</header><div className="ci-company-fact-row"><span><small>Motion</small><b>{posture.motion}</b></span><span><small>Market tier</small><b>{posture.tier}</b></span><span><small>Headquarters</small><b>{intelligence?.headquarters ?? "Research required"}</b></span><span><small>Founded</small><b>{intelligence?.founded ?? "Research required"}</b></span></div><div className="ci-company-market-body"><section><b>Markets with evidence</b><div className="ci-market-evidence-grid">{(intelligence?.industries ?? selectedCompetitor.marketRelevance).map((market) => { const matchedVertical = marketSegments.find((segment) => segment.vertical.toLowerCase().includes(market.toLowerCase()) || market.toLowerCase().includes(segment.vertical.toLowerCase()))?.vertical; const segments = matchedVertical ? marketSegments.filter((segment) => segment.vertical === matchedVertical) : []; return <button className={industryFocus === market ? "selected" : ""} onClick={() => setIndustryFocus(market)} key={market}><Factory size={14} /><span><b>{market}</b><small>{segments.length ? `${segments.length} related segments` : "Evidence mapped"}</small></span><ChevronRight size={13} /></button>; })}</div></section><section><b>Capability domains</b><div className="ci-tag-cloud">{selectedDomains.map((item) => <button key={item} onClick={() => setDomain(item)}>{item}</button>)}</div><p className="ci-market-context-note">{industryFocus === "All industries" ? posture.strength : `${industryFocus} is now the active competitive context. Open the sales brief to tailor the export.`}</p></section></div></article>

      <article className="panel ci-workflow-map"><header><div><span className="section-label">Workflow footprint</span><h2>Product relevance by buyer workflow</h2></div><button className="ci-workflow-back" onClick={() => setActiveTab("Product depth")}>Open full product depth <ArrowRight size={11} /></button></header><div>{(intelligence?.productDepth ?? selectedCompetitor.modules.map((module) => ({ family: module, depth: "Baseline only" as const, buyerUse: moduleGroup(module) }))).map((item) => <button key={item.family} onClick={() => setActiveTab("Product depth")}><span><b>{item.family}</b><small>{item.depth}</small></span><p>{item.buyerUse}</p><ArrowRight size={12} /></button>)}</div></article>
    </div>}
    ...
    {activeTab === "Sales brief" && <div className="ci-battlecard-layout">
      <article className="panel ci-battlecard-main"><header><div><span className="section-label">Sales brief</span><h2>{selectedCompetitor.name}</h2></div><div className="ci-export-controls"><label>Target market<select value={battlecardMarket} onChange={(event) => setBattlecardMarket(event.target.value)}>{battlecardMarkets.map((market) => <option key={market}>{market}</option>)}</select></label><button onClick={exportBattlecard}><Download size={13} /> Export battle card</button></div></header><div className="ci-market-export-frame"><span><small>Market angle</small><b>{battlecardFrame.angle}</b></span><span><small>Priority workflows</small><b>{battlecardFrame.priorities.join(" · ")}</b></span></div><div className="ci-battlecard-grid"><section><b>Why they win</b>{(intelligence?.whyTheyWin ?? []).map((item) => <span key={item.claim}><Check size={11} />{item.claim}</span>)}{!intelligence && <span><Info size={11} />Research pass required.</span>}</section><section><b>How Novara can compete</b>{novaraBaseline.strengths.slice(0, 4).map((item) => <span key={item}><ArrowRight size={11} />Lead with proof of {item.toLowerCase()} for {battlecardMarket.toLowerCase()} buyers when relevant.</span>)}</section><section><b>Questions to expose fit</b>{(intelligence?.questionsToTest ?? ["Which workflows, integrations and services are non-negotiable?", "What must work offline and across sites?", "What is included in the current package?"]).map((item) => <span key={item}><Search size={11} />{item}</span>)}</section><section><b>Friction signals to validate</b>{(intelligence?.pressurePoints ?? []).map((item) => <span key={item.signal}><AlertTriangle size={11} />{item.signal}</span>)}<small>Use these as discovery questions until corroborated; do not present them as proven weaknesses.</small><a className="ci-rerun-search" href={frictionSearchUrl} target="_blank" rel="noreferrer"><RefreshCw size={11} /> Rerun friction search</a></section></div></article>
