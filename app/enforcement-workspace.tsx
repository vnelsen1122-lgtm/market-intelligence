...
export function EnforcementWorkspace() {
  ...
  return <section className="enforcement-intelligence">
    <div className="enforcement-command panel"><div><ShieldCheck size={18} /><span><b>OSHA Enforcement Cases</b><small>{formatNumber(enforcementDataset.meta.cases)} cases joined by inspection number</small></span></div><mark>{formatNumber(enforcementDataset.meta.matchedInspections)} federal bulk matches</mark></div>
    ...
    <div className="enforcement-case-workbench panel"><div className="enforcement-case-list"><header><span>Employer / place</span><span>NAICS</span><span>Issued</span><span>Initial penalty</span></header>{cases.slice(0, 120).map((item) => <button className={selected?.activityNumber === item.activityNumber ? "selected" : ""} onClick={() => setSelectedActivity(item.activityNumber)} key={`${item.activityNumber}-${item.issuanceDate}-${item.employer}`}><span><b>{item.employer}</b><small>{item.city}, {item.state}</small></span><span>{item.naics || "Unmatched"}<small>{item.matchedInspection ? "OSHA bulk linked" : "Join pending"}</small></span><span>{item.issuanceDate}</span><span>{formatCurrency(item.initialPenalty)}<ChevronRight size={12} /></span></button>)}</div>{selected && <aside><mark>{selected.matchedInspection ? "Bulk linked" : "Join pending"}</mark><h3>{selected.employer}</h3><p>{selected.city}, {selected.state} · Inspection {selected.inspectionNumber}</p><div><span><b>NAICS</b>{selected.naics || "Not yet linked"}</span><span><b>Establishment size</b>{selected.establishmentSize ? formatNumber(selected.establishmentSize) : "Not supplied"}</span><span><b>Inspection type</b>{selected.inspectionType || "Not linked"}</span><span><b>Violation events</b>{selected.violationEvents}</span><span><b>Initial penalty</b>{formatCurrency(selected.initialPenalty)}</span><span><b>Issuance</b>{selected.issuanceDate}</span></div><footer><ShieldCheck size={13} /><span>Inspection number is preserved as the governing join key. Final penalties and citation standards require the citation/violation layer.</span></footer></aside>}</div>
  </section>;
}
