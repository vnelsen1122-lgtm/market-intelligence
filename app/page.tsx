...
const productDomains = {
  Sustainability: {
    summary: "Environmental compliance, carbon, ESG reporting, and supplier data increasingly converge—but buyers still evaluate them as distinct workflows.",
    buyers: "EHS · Sustainability · Operations · Finance",
    workflows: ["Environmental compliance", "Carbon accounting", "ESG disclosure", "Supplier sustainability"],
    decisionSignals: ["Regulatory exposure", "Data assurance", "Operational integration", "Reporting burden"],
  ...
  "Contractor Management": {
    summary: "Contractor intelligence spans prequalification, work readiness, site access, field safety, insurance, and financial risk.",
    buyers: "EHS · Operations · Procurement · Risk",
    workflows: ["Prequalification", "Work readiness", "Site access", "Performance monitoring"],
    decisionSignals: ["Contractor dependence", "Network coverage", "Verification depth", "Field adoption"],
...
export default function Home() {
  ...
  const supportsHeaderExport = ["Data Catalog", "Regulations", "Sources"].includes(active);
  const supportsWorkspaceReset = ["Data Catalog", "Data Operations", "Sources", "Signals", "Products"].includes(active);
      ...
      <section className="workspace">
        ...
        <div className={`content intelligence-content${active === "Competitors" ? " competitor-content" : active === "Injuries" ? " injury-content" : ""}`}>
          {!["Competitors", "Injuries", "Corporate Activity"].includes(active) && <section className="workspace-heading">
            ...
            {!["Competitors", "Injuries"].includes(active) && <div className="workspace-meta"><span><b>{active === "Sources" ? sources.length : active === "Data Operations" ? sourceRegistry.length : filtered.length}</b> {active === "Data Operations" ? "registered sources" : "records in view"}</span>{supportsWorkspaceReset && <button onClick={resetWorkspace}><RefreshCw size={14} /> Reset view</button>}</div>}
          </section>}
          ...
          {active === "Data Operations" ? (
            ...
            <>
              {active === "Data Catalog" && <section className="data-catalog-home">
                <div className="catalog-summary panel"><div><span className="panel-kicker">Cross-index architecture</span><h2>Evidence joined through governed dimensions</h2><p>Every domain declares its grain, keys, authority, cadence, and coverage before analysis.</p></div><div className="index-state"><span><b>{domainCatalog.length}</b> governed domains</span><span><b>{sharedDimensions.length}</b> shared dimensions</span><span><b>{sourceRegistry.length}</b> source contracts</span></div></div>
              ...
              {activeProductDomain && <section className="product-domain">
                ...
                <div className="product-domain-brief panel"><div><span className="panel-kicker">Product intelligence</span><h2>{selectedProductDomain}</h2><p>{activeProductDomain.summary}</p></div><span><small>Primary users</small><b>{activeProductDomain.buyers}</b></span><span><small>Workflow scope</small><b>{activeProductDomain.workflows.length} governed workflows</b></span><span><small>Decision signals</small><b>{activeProductDomain.decisionSignals.length} monitored lenses</b></span></div>
                <div className="product-domain-lanes"><article className="panel"><span className="section-label">Workflow map</span>{activeProductDomain.workflows.map((item, index) => <div key={item}><i>{String(index + 1).padStart(2, "0")}</i><b>{item}</b></div>)}</article><article className="panel"><span className="section-label">Executive decision signals</span>{activeProductDomain.decisionSignals.map((item) => <div key={item}><CircleDot size={11} /><b>{item}</b></div>)}</article></div>
                ...
                <div className="domain-roadmap panel"><ShieldCheck size={16} /><span><b>Evidence rule</b> Capability and market-strength claims require a named source, retrieval date, evidence class, and review status.</span></div>
              </section>}
              ...
              {!(["Data Catalog", "Markets", "Competitors", "Products", "Regulations", "Enforcement", "Signals"].includes(active)) && <section className="filter-bar">
              ...
              {!(["Data Catalog", "Markets", "Competitors", "Products", "Regulations", "Enforcement", "Signals"].includes(active)) && <section className="evidence-layout">
