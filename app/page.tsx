              ...
              {active === "Competitors" && <>
                <section className="competitor-workspace">
                  ...
                  <div className="competitor-dossier panel">
                    ...
                    <div className="dossier-grid"><div><span className="section-label">Company-stated platform</span><strong>{selectedCompetitor.platform}</strong><ReliabilityBadge value={selectedCompetitor.reliability} /><span className="archetype-label">{selectedCompetitor.archetype}</span></div><div><span className="section-label">Company-stated modules</span><div className="module-list">{selectedCompetitor.modules.map((module) => <span key={module}>{module}</span>)}</div><span className="section-label message-label">Current messaging tags</span><div className="message-tags">{selectedCompetitor.messagingTags.map((tag) => <span key={tag}>{tag}</span>)}</div></div></div>
                    <div className="monitor-surfaces"><span><Radar size={15} /><b>Monitored official surfaces</b><small>Future snapshots can detect product and positioning changes.</small></span>{selectedCompetitor.monitoredSurfaces.map((surface) => <a href={surface.url} target="_blank" rel="noreferrer" key={surface.label}>{surface.label}<ArrowUpRight size={12} /></a>)}</div>
